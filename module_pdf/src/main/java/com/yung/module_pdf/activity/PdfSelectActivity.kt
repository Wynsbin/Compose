...
import com.yung.module_pdf.common.PDFSelectType
...
import com.yung.module_pdf.utils.PermissionUseCase
...
private fun PdfSelectScreen(
    ...
    val pdfLocalList by vm.pdfLocalList.asStateFlow().collectAsState()
    val pdfRecentList by vm.pdfRecentList.asStateFlow().collectAsState()
    ...
    LifecycleResumeEffect(Unit) {
        ...
        if (act != null && !PermissionUseCase.hasStorageAccess(act)) {
            PermissionUseCase.requestStoragePermissions(
                activity = act,
                onGranted = {
                    vm.loadLocalPdfList()
                ...
                LazyColumn(modifier = Modifier.fillMaxSize()) {
                    items(items = list) {
                        PdfListItem(entity = it, onClickFile = {
                            fileManagerModel.openFile(context, it, pdfSelectMode)
                        }, onClickMore = {
