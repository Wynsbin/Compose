...
interface RecentFileDao {
    ...
    fun getAll(oriFormat: RecentFileFormat): Flow<List<FileInfoEntity>>
...
suspend fun RecentFileDao.insertFile(path: String?, format: RecentFileFormat?) {
    path?.let {
        format?.let {
            val file = File(path)
            if (file.exists()) {
...
suspend fun RecentFileDao.insertUri(uri: Uri?, format: RecentFileFormat?) {
